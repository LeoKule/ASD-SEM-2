...
public:
    Graph(g_type type) //Конструктор класса, который инициализирует тип графа (направленный или ненаправленный) при создании объекта.
    {
        graph_type = type;
    }
    void addVertex(int name) //Метод для добавления вершины в граф. Если вершина уже существует, выводится сообщение об этом.
    {
    ...
    void addEdge(int name1, int name2)//Метод для добавления ребра между двумя вершинами.
                                      //Если одна из вершин не существует, выводится сообщение об этом. Если граф ненаправленный, ребро добавляется в обе стороны.
    {
    ...
    void dfs(int component, int v)//Реализация поиска в глубину (DFS).
    {                             //Этот метод используется для обхода графа и помечает вершины компонентами связности (или поиском топологического порядка в случае направленного графа).
    ...
    void find_connectivity_component_undirected()//Метод для поиска компонент связности в ненаправленном графе.
    {
    ...
    void find_connectivity_component_directed()//Метод для поиска компонент сильной связности в направленном графе.
    {                                          // Внутри метода используется поиск топологического порядка и запуск DFS в этом порядке.
    ...
    void dfs_topo_order(int component, int v, std::map<int, std::vector<int>>& list)//Вспомогательный метод для поиска топологического порядка вершин.
    {
        visited.find(v)->second = component;
...
int main()//Cоздаются два объекта графа: gr (ненаправленный) и dirgr (направленный).
          //Вершины и рёбра добавляются в оба графа, а затем вызываются методы поиска компонент связности для каждого из них.
{
