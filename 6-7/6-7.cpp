...
{//ф-ия создаёт вектор рёбер, где каждое ребро представляется вектором из четырёх элементов:
 //масса ребра, начальная вершина, конечная вершина и флаг, указывающий, входит ли ребро в остовное дерево затем она сортирует рёбра по их массе
...
{//алгоритм Крускала для построения минимального остовного дерева
 //она использует отсортированный список рёбер, чтобы выбирать рёбра с наименьшей массой, не образующие циклы
    ...
    while (vertexArr.size() > 1)
    {
        ...
        {
            // тут должен быть вайл вертексарр сайз не равен 1
...
{//алгоритм Прима для построения минимального остовного дерева
 //она использует отсортированный список рёбер и поочерёдно добавляет новые вершины и рёбра в остовное дерево
    ...
    visited.push_back(unvisited[0]); //заносим любую вершину в посещенную, для алгоритма не важно с какой мы начинаем
    unvisited.erase(unvisited.begin());

    while (unvisited.size() > 0)
    {
        std::vector<int> candidates{}; //список ребер (согласно номерам в edges которые являются соседями для текцщих посещенных
        for (int i = 0; i < edges.size(); i++)
        {
